...
import Shoppingmodal from "../modal/Shopping";
...
const Shoppingmenu = ({
  ...
  const filteredMenuItems = selectedSubcategory
    ? subcategories.find((subcat) => subcat.subcategory === selectedSubcategory)
        ?.ShoppingItems || []
    : subcategories.flatMap((subcat) => subcat.ShoppingItems); // Show all if no subcategory is selected
  ...
  {filteredMenuItems.length > 0 ? (
        ...
        >
          ...
          <div className="p-5">
            ...
            <p className="mt-2 text-gray-600 line-clamp-2">
              {item.subcategory === "Shopping Malls" && item.facilities}
              {item.subcategory === "Boutiques" && item.specialization}
              {item.subcategory === "Markets" && item.marketType}
              {item.subcategory === "Antique Stores" && item.specialty}
              {item.subcategory === "Souvenir Shops" && item.itemsAvailable}
              {item.subcategory === "Outlet Stores" && item.brandsAvailable}
              {item.subcategory === "Speciality Stores" && item.speciality}
            </p>

            <div className="mt-4 flex items-center justify-between">
              {/* Show operating days for markets */}
              {item.subcategory === "Markets" && item.operatingDays ? (
                <span className="inline-flex items-center text-sm text-gray-500">
                  ...
                  {item.operatingDays.split(',').map(day => day.trim()).join(', ')}
                </span>
              ) : (
            ...
            {(item.parkingAvailability || item.foodCourt || item.securityServices) && (
              <div className="mt-3 flex flex-wrap gap-1">
                {item.parkingAvailability && (
    ...
    <div className="text-center py-16">
      ...
      <h3 className="mt-2 text-lg font-medium text-gray-900">
        No shopping venues found
      </h3>
      <p className="mt-1 text-gray-500">
        We couldn&apos;t find any {selectedSubcategory ? selectedSubcategory.toLowerCase() : "shopping"} venues.
      </p>
    </div>
  )}
</div>
